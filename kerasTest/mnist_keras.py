...
#下面把训练集中的手写黑白字体变成标准的四维形式，即（样本数量，宽，高，通道数）
#把数据变为浮点格式
...
#由于 输入层需要10个节点， 所以最好把目标数字0-9做出0one-hot编码
def tran_y(y):
    y_one = np.zeros(10)
    y_one[y] = 1
    return y_one

#把标签用0ne_hot 编码重新表示一下
...
#添加第一次卷积层，构造64个过滤器，每个过滤器覆盖范围3*3*1
